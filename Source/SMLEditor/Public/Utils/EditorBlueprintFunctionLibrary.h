		...
		if (inClass.mUnlocks != mUnlocks)
		{
			for (int32 i = 0; i < inClass.mUnlocks.Num(); i++)
			{
				...
				//UFGUnlockScannableResource
				{
					...
					{
						TArray< TSubclassOf< class UFGResourceDescriptor > > A_ = A->GetResourcesToAddToScanner();
						TArray< TSubclassOf< class UFGResourceDescriptor > > B_ = B->GetResourcesToAddToScanner();
						if (A != B)
							return false;
					}
